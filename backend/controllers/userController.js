...
// @desc    Save a remedy
// @route   POST /api/user/saved-remedies
// @access  Private
export const saveRemedy = asyncHandler(async (req, res) => {
  const user = await User.findById(req.user._id);

  if (user) {
    const { title, description, category, icon, remedies, precautions } = req.body;

    // Check if duplicate
    const alreadySaved = user.savedRemedies.find(r => r.title === title);
    if (alreadySaved) {
      res.status(400);
      throw new Error('Remedy already saved');
    }
    ...
    user.savedRemedies.push(remedy);
    await user.save();

    res.status(201).json({ message: 'Remedy saved successfully' });
  } else {
    res.status(404);
    throw new Error('User not found');
  }
...
// @desc    Get saved remedies
// @route   GET /api/user/saved-remedies
// @access  Private
export const getSavedRemedies = asyncHandler(async (req, res) => {
  const user = await User.findById(req.user._id);
...
// @desc    Delete saved remedy
// @route   DELETE /api/user/saved-remedies/:id
// @access  Private
export const deleteSavedRemedy = asyncHandler(async (req, res) => {
  const user = await User.findById(req.user._id);

  if (user) {
    user.savedRemedies = user.savedRemedies.filter(
      (remedy) => remedy._id.toString() !== req.params.id
    );
    await user.save();
